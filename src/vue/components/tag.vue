    ...
    <div :style="{...groupInfoContainer,...clipPath
      ...
      <div :style="groupInfo">
        <div ref="info">
          ...
          <div
            class="summary"
            v-for="(summary) in studentGroup.summarys"
            v-bind:key="summary.index"
          >
            {{summary.text}}
            <br />
            <br />
...
export default {
  ...
  computed: {
    ...
    tagContainer() {
      return {
        "z-index": this.expandedGroupID == this.studentGroup.groupID ? 1 : 0,
        position: "absolute",
      ...
      return {
        "clip-path": `polygon(0% 0%, 100% ${10 *
          ...
          10 *
            (1 -
              Math.min(this.now.width, this.tagData_.maxWidth) /
                Math.max(this.now.width, this.tagData_.maxWidth))}%)`
      };
    },
    groupImageSize() {
      return {
        "background-size": `${
          ...
          this.now.width /
            ...
            : (this.image.height / this.image.width) * this.now.width
        }px`
      };
    },
    groupImage() {
  ...
  methods: {
    mainLoop() {
      ...
      }
      {
        ...
        this.groupInfoHeight = infoRect.height + groupDataRect.height + 100;
      }
      {
        if (this.target.width != this.tagData_.minWidth) {
          if (this.now.width != this.target.width) {
            this.now.width = this.target.width * 0.1 + this.now.width * 0.9;
          }
          if (Math.abs(this.now.width - this.target.width) < 100) {
            this.now.height = this.target.height * 0.1 + this.now.height * 0.9;
          }
        } else {
        ...
        // }
        if (this.expandedGroupID == this.studentGroup.groupID) {
          if (Math.round(this.now.height) != Math.round(this.target.height)) {
            this.writeExpandedGroupHeigth(Math.round(this.now.height));
          }
          if (
            Math.round(this.target.height) ==
              Math.round(this.tagData_.minHeight) &&
            Math.round(this.now.height) == Math.round(this.target.height)
          ) {
