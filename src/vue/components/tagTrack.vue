        ...
        if (expandedGroupHeight != prevExpandedGroupHeight) {
          ...
          for (let i = 1; i < this.studentGroupJson.length; i++) {
            if (this.expandedGroupID == i) {
              topTable[i] = topTable[i - 1] + this.expandedGroupHeight;
            } else {
              topTable[i] = topTable[i - 1] + this.tagData.minHeight;
            }
          }
    ...
    let bindWheel = () => {
      window.setTimeout(() => {
        if (document.getElementById(`innerPage-${this.id}`)) {
          ...
          document.getElementById(`innerPage-${this.id}`).addEventListener(
            "touchmove",
            event => {
    ...
    mainLoop() {
      requestAnimationFrame(this.mainLoop);
      {
        if (this.target.top > this.init.height / 2) {
          ...
          this.target.top = Math.round(
            this.target.top * 0.8 +
              (-1 * this.studentGroupJson.length * this.tagData.minHeight +
                this.init.height / 2) *
                0.2
          );
        }
