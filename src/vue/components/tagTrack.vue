...
export default {
        ...
        if (expandedGroupHeight != prevExpandedGroupHeight) {
          ...
          for (let i = 1; i < this.studentGroupJson.length; i++) {
            if (this.expandedGroupID == i) {
              topTable[i] =
                topTable[i - 1] + this.expandedGroupHeight + this.init.spacing;
            } else {
              topTable[i] =
                topTable[i - 1] + this.tagData.minHeight + this.init.spacing;
            }
          }
  ...
  methods: {
          ...
          -1 *
            ((this.studentGroupJson.length - 1) *
              ...
              (this.expandedGroupID == -1
                ...
                : Math.max(this.expandedGroupHeight, this.tagData.maxHeight))) +
            this.init.height / 2
        ) {
          this.target.top = Math.round(
            this.target.top * 0.8 +
              (-1 *
                ((this.studentGroupJson.length - 1) * this.tagData.minHeight +
                  (this.expandedGroupID == -1
                    ? this.tagData.minHeight
    ...
    writeExpandedGroupID(id) {
      ...
      if (id != -1) {
        this.target.top = Math.round(
          -1 * (id - 1) * (this.tagData.minHeight + this.init.spacing) +
